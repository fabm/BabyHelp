...
<c:if test="${!m.validator.formHealthTec}">
    <form>
        <div>
            <cp:textbox name="mail" value="${m.inputs.mail}"/>
            <cp:errorLabel
                    isShown="${m.validator.mailEmpty}">${m.bundle.erroMailVazio}</cp:errorLabel>
            <cp:errorLabel isShown="${m.validator.mailMalformed}">${m.bundle.erroMailInvalido}</cp:errorLabel>
            <cp:errorLabel isShown="${m.validator.mailAlreadyExists}">${m.bundle.mailAlreadyExists}</cp:errorLabel>
        </div>
        <input type="submit">
    </form>
</c:if>
